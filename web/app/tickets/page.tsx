...
// ticket (card com título + texto). Autor, data/hora e assinatura ficam registrados. Admin
// vê todos, muda status (aberto/andamento/resolvido, com horários) e escreve a devolutiva.
// Paleta alinhada ao CRM (funil): navy/púrpura(wine)/azul/cinzas/branco.
...
const CATS = [
  { key: "bugs", nome: "Correção de Bugs", emoji: "🐞", desc: "Reportar erros e problemas no sistema.", cor: "#3f4a5e" },
  { key: "feature", nome: "Novas Features", emoji: "✨", desc: "Solicitar novas funcionalidades.", cor: WINE },
  { key: "supervisor", nome: "Chamado ao Supervisor", emoji: "💬", desc: "Abrir um chamado ao supervisor.", cor: "#155e9c" },
];
const ST: Record<string, { label: string; cor: string; bg: string }> = {
  aberto: { label: "Aberto", cor: "#475569", bg: "#e9edf4" },
  andamento: { label: "Em andamento", cor: "#155e9c", bg: "#e3edf8" },
  resolvido: { label: "Resolvido", cor: WINE, bg: "#f3e7ef" },
};
  ...
  return (
    <div style={{ minHeight: "100vh", background: BG, color: NAVY, fontFamily: "Inter, system-ui, sans-serif" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 20px", background: SURF, borderBottom: `1px solid ${BORDER}`, position: "sticky", top: 0, zIndex: 10 }}>
        ...
        {admin && <span style={{ marginLeft: 8, fontSize: 11, fontWeight: 800, color: "#fff", background: WINE, borderRadius: 100, padding: "3px 11px", letterSpacing: ".03em" }}>ADMIN · vê todos</span>}
      </div>

      <div style={{ maxWidth: 1180, margin: "0 auto", padding: "18px 16px 60px" }}>
        <p style={{ color: GRAY, fontSize: 13.5, margin: "0 0 18px", fontWeight: 500 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 16, alignItems: "start" }}>
          {CATS.map((cat) => {
            ...
            return (
              <section key={cat.key} style={{ background: SURF, border: `1px solid ${BORDER}`, borderRadius: 14, overflow: "hidden", boxShadow: "0 6px 20px rgba(17,29,51,.10)" }}>
                <div style={{ padding: "14px 16px", background: cat.cor }}>
                  <div style={{ fontSize: 16, fontWeight: 800, color: "#fff" }}>{cat.emoji} {cat.nome}</div>
                ...
                <div style={{ padding: 14 }}>
                  {!formAberto ? (
                    ...
                    <div style={{ border: `1px solid ${BORDER}`, borderRadius: 10, padding: 12, background: PANEL }}>
                      ...
                      <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                        <button onClick={() => setAberto(null)} disabled={salvando} style={{ marginLeft: "auto", padding: "8px 14px", fontSize: 13, fontWeight: 700, color: GRAY, background: SURF, border: `1px solid ${BORDER}`, borderRadius: 8, cursor: "pointer" }}>Cancelar</button>
                        <button onClick={() => criar(cat.key)} disabled={salvando} style={{ padding: "8px 16px", fontSize: 13, fontWeight: 800, color: "#fff", background: WINE, border: `1px solid ${WINE}`, borderRadius: 8, cursor: salvando ? "wait" : "pointer" }}>{salvando ? "Abrindo…" : "Abrir ticket"}</button>
                      </div>
                    </div>
                  )}
                  ...
                  <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 12 }}>
                    {loading ? (
                      <div style={{ fontSize: 13, color: GRAYL, padding: "8px 0", fontWeight: 500 }}>Carregando…</div>
                    ) : doCat.length === 0 ? (
                      <div style={{ fontSize: 13, color: GRAYL, padding: "8px 0", fontWeight: 500 }}>Nenhum ticket ainda.</div>
        ...
        <div style={{ marginTop: 10, paddingTop: 10, borderTop: `1px solid ${BORDER}` }}>
          {!editando ? (
            ...
            <div>
              ...
              <label style={{ fontSize: 11, fontWeight: 800, color: NAVY }}>Responsável</label>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                ...
                <button onClick={() => salvar({ devolutiva: dev, responsavel_nome: resp })} disabled={salvando} style={{ padding: "7px 14px", fontSize: 12.5, fontWeight: 800, color: "#fff", background: WINE, border: `1px solid ${WINE}`, borderRadius: 8, cursor: salvando ? "wait" : "pointer" }}>{salvando ? "Salvando…" : "Salvar devolutiva"}</button>
              </div>
            </div>
          )}
        </div>
