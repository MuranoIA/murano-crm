...
// Relatório "Clientes de Santa Bárbara" (filtro por localização — sem período).
// GET ?format=json|xlsx. Fonte: RPC relatorio_clientes_santa_barbara (wth_endereco, cidade).
...
export async function GET(req: Request) {
  ...
  const { data, error } = await sb.rpc("relatorio_clientes_santa_barbara", { p_vendedor: carteira });
  ...
  if (format === "xlsx") {
    ...
    linha("Clientes de Santa Bárbara", true, 14);
    linha(`Gerado em ${new Date().toLocaleString("pt-BR", { timeZone: "America/Sao_Paulo" })}`);
    linha(`Escopo: ${escopo}`);
    linha("");
    linha(`Total de clientes em Santa Bárbara: ${linhas.length}`, true);
    ...
    const fname = `clientes_santa_barbara_${new Date().toISOString().slice(0, 10)}.xlsx`;
  ...
  return Response.json({
    periodoLabel: "Santa Bárbara (município)",
    escopo,
