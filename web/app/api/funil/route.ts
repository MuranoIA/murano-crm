...
export async function GET() {
  ...
  const COLS_FULL = "cliente_id,cliente,vendedor,etapa,ultima_atividade,ultima_mensagem,ultima_enviada_por,telefone,ultimas_mensagens,venda_valor,venda_data,sem_cadastro,rd_cliente_id";
  ...
  const carregarCards = async (): Promise<any[]> => {
    ...
    for (let from = 0; ; from += PAGE) {
      ...
      if (error) {
        if (cols === COLS_FULL && /venda_valor|venda_data|sem_cadastro|rd_cliente_id/.test(error.message)) { cols = COLS_MSGS; from -= PAGE; continue; }
