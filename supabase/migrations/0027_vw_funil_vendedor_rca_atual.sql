-- =============================================================================
-- vw_funil atribui o card de conversa pelo RCA ATUAL do cliente (wth_vinculo ->
-- wth_carteira -> carteira_config), com fallback pra tag do RD (clientes.carteira)
-- quando não há vínculo WinThor. Reflete trocas de RCA automaticamente (decisão do
-- usuário: dono do cliente = RCA atual). A etapa continua vindo da conversa.
-- Perf verificada: ~160ms (Memoize em carteira_config). Aplicar só no murano-conversas.
-- =============================================================================
create or replace view vw_funil as
 SELECT c.id AS cliente_id, c.nome_completo AS cliente,
