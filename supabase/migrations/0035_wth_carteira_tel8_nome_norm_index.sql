-- =============================================================================
-- Performance do selo sem_cadastro: 0034 fazia seq scan da wth_carteira com
-- translate/regexp por linha (vw_funil foi de ~240ms p/ 7s). Aqui pré-computo colunas
-- GERADAS indexadas: tel8 (telefone limpo, últimos 8) e nome_norm (nome sem acento,
-- maiúsculas, espaços colapsados). A sync (wth_sync_carteira_http) usa lista de colunas
-- explícita e não escreve estas -> seguras. vw_funil passa a casar por índice (~190ms).
-- Aplicar só no murano-conversas.
-- =============================================================================
alter table wth_carteira
  add column if not exists tel8 text generated always as
    (right(regexp_replace(coalesce(telefone, ''), '\D', '', 'g'), 8)) stored,
  add column if not exists nome_norm text generated always as
    (upper(btrim(regexp_replace(translate(coalesce(nome, ''),
       'áàâãäéèêëíìîïóòôõöúùûüçÁÀÂÃÄÉÈÊËÍÌÎÏÓÒÔÕÖÚÙÛÜÇ',
       'aaaaaeeeeiiiiooooouuuucAAAAAEEEEIIIIOOOOOUUUUC'), '\s+', ' ', 'g')))) stored;

create index if not exists idx_wth_carteira_tel8 on wth_carteira (tel8);
create index if not exists idx_wth_carteira_nome_norm on wth_carteira (nome_norm);
