-- /orcamento: campanhas por PRECO UNITARIO do item (oferta_itens.vlr_regiao_unitario)
-- de TODAS as ofertas ativas do v2, inclusive combos/kits (antes so ofertas individuais
-- de 1 item, usando preco_alvo do pacote -> produtos que so vem em kit, como os NutriDivine
-- do "Saldao de Verao", ficavam sem preco de campanha). Combos recebem o sufixo " (kit)".
-- Alimenta o espelho wth_campanhas (murano-conversas); roda no cron a cada 6h.
