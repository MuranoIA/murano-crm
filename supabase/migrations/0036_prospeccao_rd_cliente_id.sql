-- =============================================================================
-- Card de prospecção deve abrir o RD Conversas quando o cliente JÁ tem contato lá
-- (mesmo nunca contatado por operador), não o WhatsApp cru. ~87% da prospecção
-- (1094/1259) tem contato no RD. Expõe rd_cliente_id = id do contato RD que casa por
-- telefone8 (prefere o que tem mensagens, depois id mais novo). Só quem não tem
-- contato no RD cai no WhatsApp. Índice idx_clientes_tel8_raw mantém ~200ms.
-- Aplicar só no murano-conversas.
-- =============================================================================
create index if not exists idx_clientes_tel8_raw on clientes (right(telefone, 8));
...
UNION ALL
 SELECT 'venda:'::text || vm.codcli::text, vm.nome, vm.carteira, vm.data_fat::timestamptz, NULL::text, NULL::text, 'pedido_emitido'::text, vm.telefone, NULL::jsonb, vm.valor_mes, vm.data_fat, false, NULL::text
